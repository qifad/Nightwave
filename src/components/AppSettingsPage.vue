...
const updateText = computed(() => {
  ...
  if (props.updateState.status === 'downloading') return '正在下载更新';
  if (props.updateState.status === 'available') return `发现 v${props.updateState.remoteVersion}`;
...
<template>
  <section class="app-settings-page" aria-label="Nightwave 设置">
    ...
    <div class="settings-page-grid">
      ...
      <section class="settings-page-card"><header><CloudDownload /><div><h2>在线更新</h2><p>{{ updateText }}</p></div></header><button class="page-action" type="button" :disabled="['checking', 'downloading'].includes(updateState.status)" @click="emit('check-update')"><RefreshCw :class="{ spin: ['checking', 'downloading'].includes(updateState.status) }" />检查更新</button></section>
